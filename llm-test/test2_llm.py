...
if input_tokens is not None and output_tokens is not None:
    ...
    if output_tokens > 0:
        ...
        print(f"  ⚡ THROUGHPUT")
        print(f"  {'─' * 40}")
        print(f"     Output tokens/sec:    {output_tokens / total_time:.1f} tok/s")
        print(f"     ms per output token:  {(total_time / output_tokens) * 1000:.1f} ms/tok")
        print(f"     Total tokens/sec:     {total_tokens / total_time:.1f} tok/s")
else:
