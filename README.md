...
This repo includes standalone benchmark scripts for comparing plain LLM behavior against `RLM` behavior on the same prompt. The current scripts use the Gemini client, so set `GEMINI_API_KEY` before running them.
...
The root of the repo contains paired scripts where the `*_llm_only.py` file runs a plain LLM and the corresponding non-`llm_only` file runs both the baseline LLM and `RLM`.

Examples:

```bash
# Under-specified TSP benchmark
python3 test_tsp_llm_only.py
python3 test_tsp_branch_bound.py

# Stochastic adaptive TSP benchmark
python3 test_stochastic_tsp_adaptive_llm_only.py
python3 test_stochastic_tsp_adaptive.py

# PDF / long-context benchmarks
python3 test_pdf_cersei_warning.py
python3 test_long_context_clinical_trial.py
python3 test_launch_note_app.py
```

### Imported `llm-test` Directory
The repo also contains a preserved copy of the external LLM-only benchmarks under [`llm-test/`](./llm-test). These are useful when you want to run the original baseline-only scripts without touching the root benchmark files.
...
### Notes
- `*_llm_only.py` scripts run a single baseline LLM completion.
- The paired `RLM` scripts run a baseline LLM first and then an `RLM` run on the same prompt.
- Most of these scripts print prompt text, wall time, and token usage directly to the terminal.

### Latest Findings
- The cheapest matched benchmark is the under-specified TSP pair:
  - `python3 test_tsp_llm_only.py`
  - `python3 test_tsp_branch_bound.py`
- In the latest observed run on `gemini-2.5-flash`, the baseline LLM request stalled in the Gemini API call and had to be interrupted, while the equivalent `RLM` reasoning path completed and correctly identified that the prompt could not be solved because the distance matrix was missing.
- The latest `RLM`-only run on that prompt completed in about `20.25s` with `24,367` input tokens and `1,006` output tokens, and produced a grounded refusal rather than inventing missing data.
- A rerun note with exact commands and the observed result is available in [`benchmark_rerun_tsp.md`](./benchmark_rerun_tsp.md).
- For the stochastic adaptive TSP benchmark, the latest observed `RLM` run on `test_stochastic_tsp_adaptive.py` completed in `73.378s` with `14,740` input tokens and `3,328` output tokens (`18,068` total).
- A rerun note for that benchmark is available in [`benchmark_rerun_stochastic_tsp.md`](./benchmark_rerun_stochastic_tsp.md).
- For the auth proxy long-context benchmark, the latest matched run on `gemini-2.5-flash-lite` clearly favored the baseline `LLM`:
  - the baseline `LLM` answered all five questions correctly in `1.127s` using `1,443` input tokens and `66` output tokens
  - the latest `RLM` run on [`test_long_context_authproxy.py`](./test_long_context_authproxy.py) did not cleanly answer the Q1-Q5 benchmark; it summarized the document, claimed the user query was missing, and only indirectly recovered the relevant facts, finishing in `13.941s` wall time with `15,330` input tokens and `1,628` output tokens
- A rerun note for that benchmark is available in [`benchmark_rerun_authproxy.md`](./benchmark_rerun_authproxy.md).
